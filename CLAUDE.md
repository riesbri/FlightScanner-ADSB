...
- **`notification/DiscordFlightNotifier`** — sends webhook embeds. Category (and therefore emoji + color) is taken from the shared `AircraftTypes.AircraftCategory` enum, so the "is this interesting?" filter and the embed emoji can never disagree.
