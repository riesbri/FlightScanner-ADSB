...
import com.flightscanner.service.AircraftEnrichmentService;
...
public class Dump1090DataSource implements ADSBDataSource, AutoCloseable {
    ...
    // Aircraft tracking state — capped to prevent unbounded growth from spoofed SBS feeds
    private static final int MAX_TRACKED_AIRCRAFT = 5000;
    ...
    private void processMessage(String line) {
        ...
        if (isNew && aircraftMap.size() >= MAX_TRACKED_AIRCRAFT) {
            log.warn("aircraftMap at capacity ({}); dropping new hex {}", MAX_TRACKED_AIRCRAFT, key);
            return;
        }
